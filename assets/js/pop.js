        ...
        xhr.onload = function () {
            if (this.status >= 200 && this.status < 300) {
              ...
              for(var i=0;i<response.length;i++){
                if(response[i]['trait_type'] == 'personhood_score'){
                    resolve(response[i]['value']);
                }
              }
              resolve(0);
            } else {
...
async function get_personhoodscore(web3, network, address){
    ...
    try{
        ...
        // query contract
        let balance = await passport.methods.balanceOf(address).call();
        if(!balance){
            return 0;
        }
        ...
        var result = await makeRequest('GET', tokenURI);
        return result;
    } catch (err){
        return 0
    }
